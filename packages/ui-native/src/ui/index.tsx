...
type UiProps = { children?: ReactNode; style?: StyleProp<ViewStyle>; title?: string };

function UiBlock({ children, style, title }: UiProps) {
  return (
    <View style={[{ borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 8, padding: 8 }, style]}>
      <Text style={{ marginBottom: 6, fontWeight: "700" }}>{title ?? "UI"}</Text>
      {children}
    </View>
  );
}
