...
export function ConfirmStep(p: ConfirmProps) {
  return (
    <div className="space-y-4 px-5 py-5">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[1.75rem] border border-border bg-card shadow-[0_8px_24px_rgba(0,0,0,0.06)]">
        <div className="relative h-40">
          ...
          <div className="absolute bottom-4 left-4 right-4 text-white">
            <p className="truncate text-[1.25rem]" style={{ fontWeight: 900 }}>{p.restaurant.name}</p>
            ...
            <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-white/18 px-2.5 py-1 backdrop-blur">
              ...
              <span className="text-[0.75rem]" style={{ fontWeight: 900 }}>{fmtR(p.restaurant.rating)}</span>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <SectionCard>
        <h4 className="mb-3 text-[1rem]" style={{ fontWeight: 900 }}>Reservation</h4>
      ...
      <SectionCard>
        <h4 className="mb-3 text-[1rem]" style={{ fontWeight: 900 }}>Contact</h4>
        <div className="grid gap-2">
      ...
      {p.allPrefTags.length > 0 && (
        <SectionCard>
          <div className="mb-3 flex items-center justify-between">
            <h4 className="text-[1rem]" style={{ fontWeight: 900 }}>Preferences</h4>
            <span className="rounded-full bg-primary/10 px-2.5 py-1 text-[0.6875rem] text-primary" style={{ fontWeight: 900 }}>{p.allPrefTags.length} selected</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {p.allPrefTags.map((tag) => (
              <span key={tag.label} className="rounded-full border border-border bg-secondary/55 px-3 py-1.5 text-[0.8125rem]" style={{ fontWeight: 800 }}>
      ...
      <SectionCard>
        <h4 className="mb-3 flex items-center gap-2 text-[1rem]" style={{ fontWeight: 900 }}>
        ...
        <div className="space-y-2 text-[0.875rem]">
          <div className="flex justify-between">
            ...
            <span style={{ fontWeight: 800 }}>${p.depositAmount.toFixed(2)}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="mt-3 flex items-center justify-between rounded-[1rem] bg-primary/8 px-3 py-2.5">
            ...
            <span className="text-[1.125rem] text-primary" style={{ fontWeight: 900 }}>${p.totalAmount.toFixed(2)}</span>
          </div>
        </div>
      </SectionCard>
      ...
      <div className="flex gap-3 rounded-[1.25rem] border border-info/25 bg-info/8 p-4">
        ...
        <div>
          <p className="text-[0.8125rem] text-foreground" style={{ fontWeight: 900 }}>Refund policy</p>
...
export function AwaitingStep({
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
      ...
      <h2 className="mb-2 text-[1.5rem]" style={{ fontWeight: 900 }}>Awaiting approval</h2>
      ...
      <div className="flex w-full max-w-sm items-center gap-3 rounded-[1.5rem] border border-border bg-card p-3 text-left shadow-[0_8px_24px_rgba(0,0,0,0.06)]">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[0.875rem]" style={{ fontWeight: 900 }}>{restaurant.name}</p>
          <p className="text-[0.75rem] text-muted-foreground">{dateStr} - {selectedTime} - {guests} guests</p>
        </div>
        <div className="shrink-0 text-right">
          <p className="text-[0.875rem]" style={{ fontWeight: 900 }}>${totalAmount.toFixed(2)}</p>
          <p className="text-[0.6875rem] text-success" style={{ fontWeight: 900 }}>Paid</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="px-5 py-6">
      ...
      <div className="mb-6 flex flex-col items-center text-center">
        ...
        <h2 className="mb-1 text-[1.5rem]" style={{ fontWeight: 900 }}>Reservation approved</h2>
        ...
        <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-success/10 px-3 py-1.5 text-success">
          ...
          <span className="text-[0.8125rem]" style={{ fontWeight: 900 }}>${totalAmount.toFixed(2)} paid</span>
          <span className="rounded-full bg-warning/10 px-2 py-0.5 text-[0.6875rem] text-warning" style={{ fontWeight: 900 }}>+{POINTS_EARN} pts</span>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-[1.75rem] border border-border bg-card shadow-[0_14px_36px_rgba(0,0,0,0.09)]">
        <div className="relative h-36">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/78 via-black/20 to-transparent" />
          <div className="absolute bottom-4 left-4 right-4 text-white">
            <p className="truncate text-[1.125rem]" style={{ fontWeight: 900 }}>{restaurant.name}</p>
            <p className="mt-0.5 text-[0.8125rem] text-white/78">{dateStr} - {selectedTime}</p>
          </div>
        </div>
        <div className="space-y-4 p-5">
          <div className="text-center">
            <p className="text-[0.6875rem] uppercase tracking-[0.08em] text-muted-foreground" style={{ fontWeight: 900 }}>Confirmation</p>
            <div className="mt-1 flex items-center justify-center gap-2">
              <p className="text-[1.25rem] tracking-[0.04em]" style={{ fontWeight: 900 }}>{bookingId}</p>
          ...
          {occasion && (
            <div className="flex justify-center">
              <span className="rounded-full border border-border bg-secondary/55 px-3 py-1.5 text-[0.8125rem]" style={{ fontWeight: 900 }}>
          ...
          {allPrefTags.length > 0 && (
            <div className="flex flex-wrap justify-center gap-1.5">
              {allPrefTags.map((tag) => (
                <span key={tag.label} className="rounded-full bg-primary/8 px-2.5 py-1 text-[0.75rem] text-primary" style={{ fontWeight: 800 }}>
                  {tag.emoji ? `${tag.emoji} ` : ""}{tag.label}
                </span>
              ))}
            </div>
...
function SummaryTile({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-[1rem] bg-secondary/65 px-3 py-2 text-center">
      <p className="text-[0.6875rem] text-muted-foreground" style={{ fontWeight: 800 }}>{label}</p>
      <p className={`mt-0.5 truncate text-[0.875rem] ${accent ? "text-success" : "text-foreground"}`} style={{ fontWeight: 900 }}>{value}</p>
    </div>
  );
}
