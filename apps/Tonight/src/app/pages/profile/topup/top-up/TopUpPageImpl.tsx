...
export function TopUpPage({ onBack }: { onBack: () => void }) {
  ...
  const handleConfirm = () => {
    if (selectedProvider === "bank") { setOtp(["", "", "", ""]); setOtpError(null); setResendIn(30); setStep("auth"); } else { setStep("processing"); }
  };
  const submitOtp = () => {
    const code = otp.join("");
    if (code.length < 4) { setOtpError("Enter the 4-digit code"); return; }
    if (code === "0000") { setOtpError("That code didn't match. Try again."); return; }
    setOtpError(null); setStep("processing");
  };
  const setOtpDigit = (i: number, v: string) => {
    const digit = v.replace(/\D/g, "").slice(-1);
    setOtp((prev) => { const next = [...prev]; next[i] = digit; return next; });
    if (digit && i < 3) document.getElementById(`topup-otp-${i + 1}`)?.focus();
  };
  const pressKey = (k: string) => {
    if (k === "back") { setCustomAmount((prev) => (prev !== "" ? prev : String(amount)).slice(0, -1)); return; }
    if (k === ".") {
      if (currency !== "USD") return;
      setCustomAmount((prev) => { const base = prev !== "" ? prev : String(amount); if (base.includes(".")) return base; return (base || "0") + "."; });
      return;
    }
