    ...
    <div className="pb-8">
      ...
      <div className="flex items-stretch gap-1 mb-4 p-1.5 rounded-full bg-secondary/40 w-full">
        {CATEGORY_OPTIONS.map((c) => {
          ...
          return (
            <button key={c.id} onClick={() => setCategory(c.id)} aria-label={c.label} className={`group relative flex items-center justify-center h-12 rounded-full cursor-pointer min-w-0 overflow-hidden transition-[flex-grow,flex-basis,padding,background-color,border-color,box-shadow,color] duration-[1400ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${isActive ? "flex-[0_0_auto] px-5 bg-background border border-border shadow-sm text-foreground" : "flex-[1_1_0%] px-0 bg-transparent border border-transparent text-muted-foreground hover:text-foreground hover:bg-background/60"}`} style={{ fontWeight: isActive ? 600 : 500 }}>
              <Icon className={`w-[1.125rem] h-[1.125rem] shrink-0 transition-colors duration-[1400ms] ${isActive ? c.color : ""}`} />
              <span className="text-[0.8125rem] whitespace-nowrap inline-block transition-[max-width,opacity,margin-left,transform] duration-[1400ms] ease-[cubic-bezier(0.22,1,0.36,1)] overflow-hidden" style={{ maxWidth: isActive ? "10rem" : "0px", opacity: isActive ? 1 : 0, marginLeft: isActive ? "0.5rem" : "0px", transform: isActive ? "translateX(0)" : "translateX(-4px)" }}>{c.label}</span>
            </button>
          );
        })}
      </div>
                ...
                {g.items.map((t, idx) => (
                  <button key={t.id} onClick={() => setSelectedTransaction(t)} className={`w-full flex items-center justify-between py-3 px-3 hover:bg-secondary/30 transition-colors ${idx > 0 ? "border-t border-border" : ""}`}>
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${t.category === "charge" ? "bg-success/10" : t.category === "gift" ? "bg-success/10" : t.category === "reward" ? "bg-primary/10" : t.category === "referral" ? "bg-info/10" : "bg-secondary"}`}>
                        {t.category === "charge" ? <Plus className="w-4 h-4 text-success" /> : t.category === "gift" ? <Gift className="w-4 h-4 text-success" /> : t.category === "reward" ? <Star className="w-4 h-4 text-primary" /> : t.category === "referral" ? <Send className="w-4 h-4 text-info" /> : <ShoppingBag className="w-4 h-4 text-muted-foreground" />}
                      </div>
                      <div className="text-left min-w-0">
                        <Text className="text-[0.8125rem] truncate" style={{ fontWeight: 500 }}>{t.label}</Text>
                        <div className="flex items-center gap-1.5"><Text className="text-muted-foreground text-[0.6875rem]">{t.time}</Text><span className="text-muted-foreground text-[0.6875rem]">·</span><DSBadge variant="soft" size="sm" color={t.category === "charge" ? "success" : t.category === "reward" ? "primary" : t.category === "referral" ? "info" : t.category === "gift" ? "success" : "secondary"}>{CATEGORY_OPTIONS.find((c) => c.id === t.category)?.label}</DSBadge></div>
                      </div>
                    </div>
