          ...
          <div className="relative h-[6.75rem] overflow-hidden rounded-[1.125rem] bg-secondary">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/55 to-transparent p-2">
              <Text className="truncate text-[0.6875rem] text-white/90" style={{ fontWeight: 800 }}>
                {booking.cuisine.split(/\s*·\s*/)[0].trim()}
              </Text>
            </div>
          </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <Text className="mt-0.5 truncate text-[0.8125rem] text-muted-foreground">{booking.cuisine}</Text>
            </div>
          </div>
      ...
      <div className="border-t border-border/70 px-3.5 py-3">
        {isScheduled && (
          <div className="mb-3 flex items-center justify-between gap-2 rounded-[1rem] bg-primary/6 px-3 py-2">
            ...
            <button type="button" onClick={handleCopy} className={`relative flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center overflow-visible rounded-full bg-card shadow-sm transition ${copied ? "text-primary ring-2 ring-primary/20" : "text-muted-foreground hover:text-foreground"}`} aria-label="Copy confirmation code">
              <AnimatePresence mode="wait" initial={false}>
                {copied ? (
                  <motion.span key="check" initial={{ scale: 0.35, rotate: -30, opacity: 0 }} animate={{ scale: [1.15, 0.95, 1], rotate: 0, opacity: 1 }} exit={{ scale: 0.4, opacity: 0 }} transition={{ duration: 0.34, ease: "easeOut" }}>
                    <CheckCircle2 className="h-4.5 w-4.5" />
                  </motion.span>
                ) : (
                  <motion.span key="copy" initial={{ scale: 0.75, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.6, opacity: 0 }} transition={{ duration: 0.16 }}>
              ...
              <AnimatePresence>
                {copied && (
                  <>
                    <motion.span className="absolute inset-0 rounded-full border border-primary" initial={{ scale: 0.8, opacity: 0.5 }} animate={{ scale: 1.9, opacity: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.55, ease: "easeOut" }} />
                    <motion.span className="absolute -top-9 right-0 whitespace-nowrap rounded-full bg-foreground px-2.5 py-1 text-[0.6875rem] text-background shadow-lg" initial={{ opacity: 0, y: 5, scale: 0.92 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 5, scale: 0.92 }} transition={{ duration: 0.18 }} style={{ fontWeight: 800 }}>
                      Copied
                    </motion.span>
                  </>
                )}
        ...
        {isVisited && (
          <div className="mb-3 grid grid-cols-3 gap-2">
            <div className="rounded-[1rem] bg-secondary/65 px-3 py-2">
              <Text className="text-[0.6875rem] text-muted-foreground" style={{ fontWeight: 700 }}>Rating</Text>
              <div className="mt-0.5 flex items-center gap-1 text-warning">
                <Star className="h-3.5 w-3.5 fill-warning" />
                <Text className="text-[0.875rem] text-warning" style={{ fontWeight: 800 }}>{booking.rating ? fmtR(booking.rating) : "--"}</Text>
              </div>
            </div>
            <div className="rounded-[1rem] bg-secondary/65 px-3 py-2">
              <Text className="text-[0.6875rem] text-muted-foreground" style={{ fontWeight: 700 }}>Paid</Text>
              <Text className="mt-0.5 truncate text-[0.875rem]" style={{ fontWeight: 800 }}>{receiptTotal ?? "--"}</Text>
            </div>
            <div className="rounded-[1rem] bg-secondary/65 px-3 py-2">
              <Text className="text-[0.6875rem] text-muted-foreground" style={{ fontWeight: 700 }}>Points</Text>
              <Text className="mt-0.5 truncate text-[0.875rem]" style={{ fontWeight: 800 }}>+{booking.diningPoints}</Text>
            </div>
          </div>
        )}
        ...
        {isCancelled && (
          <div className="mb-3 flex items-start gap-2 rounded-[1rem] bg-secondary/65 px-3 py-2">
            ...
            <Text className="line-clamp-2 text-[0.8125rem] leading-snug text-muted-foreground">{booking.address}</Text>
          </div>
        )}

        <div className="flex items-center gap-2">
          <PrimaryAction booking={booking} isLiveDining={isLive} onOpenLive={onTap} onManage={onManage} onScanQR={onScanQR} onBookAgain={onBookAgain} onViewReceipt={onViewReceipt} />

          {isScheduled && (
            <>
              <Button variant="outline" size="sm" radius="full" className="min-h-9 px-3 font-bold" onClick={stop(onManage)}>Manage</Button>
              <button type="button" onClick={stop(onInvite)} className="relative flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full border border-border bg-card text-foreground transition hover:bg-secondary" aria-label="Invite friends">
                <UserPlus className="h-4 w-4" />
                {invitedCount > 0 && <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-primary px-1 text-[0.5625rem] text-primary-foreground">{invitedCount}</span>}
              </button>
              <button type="button" onClick={stop(onShowQR)} className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full border border-border bg-card text-foreground transition hover:bg-secondary" aria-label="Show QR">
