...
import { DateTimePicker, isSlotAvailable, formatChipLabel, type DateTimeValue } from "./DateTimePicker";
...
export function ExplorerPage() {
  ...
  const [maxPanelHeight, setMaxPanelHeight] = useState(() => typeof window !== "undefined" ? window.innerHeight * 0.7 : 500);
  const collapsedHeight = 72;
  useEffect(() => {
    const onResize = () => setMaxPanelHeight(window.innerHeight * 0.7);
    window.addEventListener("resize", onResize);
    return () => window.removeEventListener("resize", onResize);
  }, []);
  const panelHeight = useMotionValue(collapsedHeight);
  const dragStartHeight = useRef(collapsedHeight);
  const [showListContent, setShowListContent] = useState(false);
  useMotionValueEvent(panelHeight, "change", (h) => {
    setShowListContent(h > collapsedHeight + 20);
  });
  const SPRING = { type: "spring" as const, stiffness: 340, damping: 34, mass: 0.9 };
  useEffect(() => {
    const target = listExpanded ? maxPanelHeight : collapsedHeight;
    const controls = animate(panelHeight, target, SPRING);
    return () => controls.stop();
  }, [listExpanded, maxPanelHeight]);
  const handlePanStart = () => { dragStartHeight.current = panelHeight.get(); };
  const handlePan = (_: unknown, info: PanInfo) => {
    let next = dragStartHeight.current - info.offset.y;
    if (next < collapsedHeight) next = collapsedHeight - Math.pow(collapsedHeight - next, 0.7);
    if (next > maxPanelHeight) next = maxPanelHeight + Math.pow(next - maxPanelHeight, 0.7);
    panelHeight.set(next);
  };
  const handlePanEnd = (_: unknown, info: PanInfo) => {
    const current = panelHeight.get();
    const velY = info.velocity.y;
    const projected = current - velY * 0.15;
    const mid = (collapsedHeight + maxPanelHeight) / 2;
    let expand = projected > mid;
    if (velY < -600) expand = true;
    else if (velY > 600) expand = false;
    setListExpanded(expand);
    animate(panelHeight, expand ? maxPanelHeight : collapsedHeight, { ...SPRING, velocity: -velY });
  };
  ...
  const [dateTime, setDateTime] = useState<DateTimeValue>({ date: null, time: null, party: 2 });
  ...
  const filteredRestaurants = RESTAURANTS.filter(r => {
    ...
    if (dateTime.date && dateTime.time && !isSlotAvailable(r.id, dateTime.date, dateTime.time, dateTime.party)) return false;
    return true;
  });
  ...
  const drawerTitle = drawerMode === "sort" ? "Sort By" : drawerMode === "price" ? "Price Range" : drawerMode === "cuisine" ? "Cuisine" : drawerMode === "amenities" ? "Amenities" : drawerMode === "datetime" ? "Time & Date" : "All Filters";
  ...
  return (
    <div className="relative w-full h-full min-h-[calc(100vh-64px)] lg:min-h-screen flex flex-col">
      ...
      {showActionPanel && selectedR ? (
        ...
        <motion.div className="absolute left-0 right-0 z-30 bg-card border-t border-border rounded-t-2xl shadow-2xl overflow-hidden bottom-[calc(env(safe-area-inset-bottom,0px)+52px)] lg:bottom-0" style={{ height: panelHeight }}>
          <motion.div className="select-none" onPanStart={handlePanStart} onPan={handlePan} onPanEnd={handlePanEnd} style={{ touchAction: "pan-x" }}>
            ...
            <div ref={filterScrollRef} className="flex items-center gap-2 px-3 pb-2.5 overflow-x-auto scrollbar-hide select-none">
              ...
              <FilterBarChip label={formatChipLabel(dateTime)} hasChevron active={!!(dateTime.date || dateTime.time)} onClick={() => setDrawerMode("datetime")} />
      ...
      {/* ── Filter Drawer ── */}
      <Drawer open={drawerMode !== null} onClose={() => setDrawerMode(null)} side="bottom" size={drawerMode === "all" || drawerMode === "datetime" ? "xl" : "md"} title={drawerTitle} className={drawerMode === "all" ? "!max-h-[85vh]" : drawerMode === "datetime" ? "!max-h-[85vh]" : drawerMode === "amenities" ? "!h-auto !max-h-[60vh]" : "!h-auto !max-h-[50vh]"} footer={<div className="flex gap-3 w-full">{drawerMode === "all" && <Button variant="outline" onClick={resetFilters} className="flex-1 gap-2"><RotateCcw className="w-4 h-4" /> Reset</Button>}{drawerMode === "datetime" && (dateTime.date || dateTime.time) && <Button variant="outline" onClick={() => setDateTime({ date: null, time: null, party: 2 })} className="flex-1 gap-2"><RotateCcw className="w-4 h-4" /> Clear</Button>}<Button variant="primary" onClick={() => setDrawerMode(null)} className={(drawerMode === "all" || (drawerMode === "datetime" && (dateTime.date || dateTime.time))) ? "flex-[2]" : "flex-1"}>Apply</Button></div>}>
        <DrawerBody className="space-y-5 !px-5">
          {drawerMode === "datetime" && <DateTimePicker value={dateTime} onChange={setDateTime} />}
