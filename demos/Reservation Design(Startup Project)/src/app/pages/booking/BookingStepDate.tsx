    ...
    <div className="px-4 py-4 space-y-6">
      ...
      <div>
        ...
        <div className="flex items-stretch gap-3">
          <DragScrollContainer className="flex gap-2 pb-1 flex-1 min-w-0">
            {DAYS.map((d, i) => (
              <button key={i} onClick={() => { setCustomDate(null); setSelectedDate(i); }}
                className={`flex flex-col items-center px-3 py-2.5 rounded-xl shrink-0 cursor-pointer transition-all min-w-[4rem] ${!customDate && selectedDate === i ? "bg-primary text-primary-foreground shadow-md" : "bg-card border border-border hover:bg-secondary"}`}>
          ...
          <button onClick={() => setShowCustomDatePicker(true)}
            className={`flex flex-col items-center justify-center px-3 py-2.5 mb-1 rounded-xl shrink-0 cursor-pointer transition-all min-w-[4rem] ${customDate ? "bg-primary text-primary-foreground shadow-md" : "bg-card border border-dashed border-border hover:bg-secondary"}`}>
        ...
        {showCustomDatePicker && (
          <CustomDatePickerModal value={customDate} onSelect={(d) => { setCustomDate(d); setShowCustomDatePicker(false); }} onClose={() => setShowCustomDatePicker(false)} />
        )}
      </div>
      <div>
...
export function DetailsStep({ name, setName, phone, setPhone, notes, setNotes, occasion, setOccasion }: {
  ...
  return (
    <div className="px-4 py-4 space-y-6">
      <div className="space-y-4">
        ...
        <div>
          <label className="text-[0.8125rem] mb-1.5 block text-muted-foreground" style={{ fontWeight: 500 }}>Full Name *</label>
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter your name" className="w-full px-4 py-3 rounded-xl border border-border bg-card text-[0.875rem] outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition" />
        </div>
        <div>
          <label className="text-[0.8125rem] mb-1.5 block text-muted-foreground" style={{ fontWeight: 500 }}>Phone Number *</label>
          <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="(000) 000-0000" className="w-full px-4 py-3 rounded-xl border border-border bg-card text-[0.875rem] outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition" />
        </div>
      </div>
      <div>
